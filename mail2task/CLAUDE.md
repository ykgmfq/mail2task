...
- [tasks.py](tasks.py) — `create_task()` / `build_comment()` / `add_attachment_comments()` over the official `todoist-api-python` SDK (`add_task` + `add_comment` + file uploads). These take only what they need (`project_id`, `api_token`) rather than the whole config. `create_task` sets `due_date=today`, `deadline_date=<extracted>`, `priority=2` when a deadline exists (else default), and the `mail2task` label.
